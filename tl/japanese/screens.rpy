translate japanese strings:

    # game/screens.rpy:261
    old "Back"
    new "ロールバック"

    # game/screens.rpy:262
    old "History"
    new "ヒストリー"

    # game/screens.rpy:263
    old "Skip"
    new "スキップ"

    # game/screens.rpy:264
    old "Auto"
    new "オート"

    # game/screens.rpy:265
    old "Save"
    new "セーブ"

    # game/screens.rpy:266
    old "Q.Save"
    new "Q.セーブ"

    # game/screens.rpy:267
    old "Q.Load"
    new "Q.ロード"

    # game/screens.rpy:268
    old "Prefs"
    new "設定「せってい」"

    # game/screens.rpy:309
    old "Start"
    new "スタート"

    # game/screens.rpy:317
    old "Load"
    new "ロード"

    # game/screens.rpy:319
    old "Preferences"
    new "環境設定"

    # game/screens.rpy:323
    old "End Replay"
    new "リプレイ終了「しゅうりょう」"

    # game/screens.rpy:327
    old "Main Menu"
    new "メインメニュー"

    # game/screens.rpy:329
    old "About"
    new "バージョン情報\n「じょうほう」"

    # game/screens.rpy:334
    old "Help"
    new "ヘルプ"

    # game/screens.rpy:339
    old "Quit"
    new "終了「しゅうりょう」"

    # game/screens.rpy:482
    old "Return"
    new "戻る「もどる」"

    # game/screens.rpy:566
    old "Version [config.version!t]\n"
    new "Version [config.version!t]\n"

    # game/screens.rpy:572
    old "Made with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"
    new "Made with {a=https://ja.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"

    # game/screens.rpy:612
    old "Page {}"
    new "ページ {}"

    # game/screens.rpy:612
    old "Automatic saves"
    new "オートセーブ"

    # game/screens.rpy:612
    old "Quick saves"
    new "クイックセーブ"

    # game/screens.rpy:654
    old "{#file_time}%A, %B %d %Y, %H:%M"
    new "{#file_time}%Y年%m月%d日(%a) %H時%M分"

    # game/screens.rpy:654
    old "empty slot"
    new "空「から」のスロット"

    # game/screens.rpy:671
    old "<"
    new "<"

    # game/screens.rpy:674
    old "{#auto_page}A"
    new "{#auto_page}A"

    # game/screens.rpy:677
    old "{#quick_page}Q"
    new "{#quick_page}Q"

    # game/screens.rpy:683
    old ">"
    new ">"

    # game/screens.rpy:745
    old "Display"
    new "ディスプレイ"

    # game/screens.rpy:746
    old "Window"
    new "ウィンドウ"

    # game/screens.rpy:747
    old "Fullscreen"
    new "フルスクリーン"

    # game/screens.rpy:759
    old "Unseen Text"
    new "未読「みどく」テキストも\nスキップ"

    # game/screens.rpy:760
    old "After Choices"
    new "選択肢後\n「せんたくしご」もスキップ\n継続「けいぞく」"

    # game/screens.rpy:766
    old "Examples"
    new "Examples"

    # game/screens.rpy:775
    old "Language"
    new "言語「げんご」"

    # game/screens.rpy:798
    old "Text Speed"
    new "文字表示速度\n「もじひょうじそくど」"

    # game/screens.rpy:802
    old "Auto-Forward Time"
    new "オート待「ま」ち時間「じかん」"

    # game/screens.rpy:809
    old "Music Volume"
    new "音楽{rt}おんりょう」の音量「おんりょう」"

    # game/screens.rpy:816
    old "Sound Volume"
    new "効果音{rt}こうかおん」の音量「おんりょう」"

    # game/screens.rpy:822
    old "Test"
    new "テスト"

    # game/screens.rpy:826
    old "Voice Volume"
    new "ボイスの音量「おんりょう」"

    # game/screens.rpy:837
    old "Mute All"
    new "全「ぜん」てミュート"

    # game/screens.rpy:956
    old "The dialogue history is empty."
    new "ヒストリーはありません。"

    # game/screens.rpy:1022
    old "Keyboard"
    new "キーボード"

    # game/screens.rpy:1023
    old "Mouse"
    new "マウス"

    # game/screens.rpy:1026
    old "Gamepad"
    new "ゲームパッド"

    # game/screens.rpy:1039
    old "Enter"
    new "Enter"

    # game/screens.rpy:1040
    old "Advances dialogue and activates the interface."
    new "台詞「せりふ」を読「どく」み進「しん」める。またはボタンを選択「せんたく」する。"

    # game/screens.rpy:1043
    old "Space"
    new "Space"

    # game/screens.rpy:1044
    old "Advances dialogue without selecting choices."
    new "台詞「せりふ」を読「どく」み進「しん」める。ただしボタンは選択「せんたく」しない。"

    # game/screens.rpy:1047
    old "Arrow Keys"
    new "方向「ほうこう」キー"

    # game/screens.rpy:1048
    old "Navigate the interface."
    new "インターフェースを移動「いどう」する。"

    # game/screens.rpy:1051
    old "Escape"
    new "Escape"

    # game/screens.rpy:1052
    old "Accesses the game menu."
    new "ゲームメニューを開「ひら」く。"

    # game/screens.rpy:1055
    old "Ctrl"
    new "Ctrl"

    # game/screens.rpy:1056
    old "Skips dialogue while held down."
    new "押「お」し続「ぞ」けている間「あいだ」スキップする。"

    # game/screens.rpy:1059
    old "Tab"
    new "Tab"

    # game/screens.rpy:1060
    old "Toggles dialogue skipping."
    new "スキップモードに切「き」り替「くらが」える。"

    # game/screens.rpy:1063
    old "Page Up"
    new "Page Up"

    # game/screens.rpy:1064
    old "Rolls back to earlier dialogue."
    new "前「さき」の台詞「せりふ」に戻「もど」る。"

    # game/screens.rpy:1067
    old "Page Down"
    new "Page Down"

    # game/screens.rpy:1068
    old "Rolls forward to later dialogue."
    new "ロールバック中「じゅう」、次「つぎ」の台詞「せりふ」に進「しん」む。"

    # game/screens.rpy:1072
    old "Hides the user interface."
    new "インターフェースを隠「かく」す。"

    # game/screens.rpy:1076
    old "Takes a screenshot."
    new "スクリーンショットを撮「つま」る。"

    # game/screens.rpy:1080
    old "Toggles assistive {a=https://www.renpy.org/l/voicing}self-voicing{/a}."
    new "{a=https://www.renpy.org/l/voicing}セルフボイシング{/a}を有効化「ゆうこうか」する。"

    # game/screens.rpy:1086
    old "Left Click"
    new "左「ひだり」クリック"

    # game/screens.rpy:1090
    old "Middle Click"
    new "中「ちゅう」クリック"

    # game/screens.rpy:1094
    old "Right Click"
    new "右「みぎ」クリック"

    # game/screens.rpy:1098
    old "Mouse Wheel Up\nClick Rollback Side"
    new "マウスホイール上回転「うえかいてん」\n画面「がめん」サイドをタッチ"

    # game/screens.rpy:1102
    old "Mouse Wheel Down"
    new "マウスホイール下回転「したかいてん」"

    # game/screens.rpy:1109
    old "Right Trigger\nA/Bottom Button"
    new "Ｒトリガー\nＡ／下ボタン"

    # game/screens.rpy:1113
    old "Left Trigger\nLeft Shoulder"
    new "Ｌトリガー\nＬボタン"

    # game/screens.rpy:1117
    old "Right Shoulder"
    new "Ｒボタン"

    # game/screens.rpy:1121
    old "D-Pad, Sticks"
    new "方向「ほうこう」パッド\n左右「さゆう」スティック"

    # game/screens.rpy:1125
    old "Start, Guide"
    new "スタート、ガイド"

    # game/screens.rpy:1129
    old "Y/Top Button"
    new "Ｙ／上「上」ボタン"

    # game/screens.rpy:1132
    old "Calibrate"
    new "キャリブレート"

    # game/screens.rpy:1197
    old "Yes"
    new "はい"

    # game/screens.rpy:1198
    old "No"
    new "いいえ"

    # game/screens.rpy:1244
    old "Skipping"
    new "スキップ中「ちゅう」"

    # game/screens.rpy:1465
    old "Menu"
    new "メニュー"

    # game/screens.rpy:1083
    old "Opens the accessibility menu."
    new "アクセス性「せい」メニューを開「ひら」きます。"

    # game/screens.rpy:777
    old " "
    new " "

